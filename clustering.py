...
def make_clusters(env_name, n_clusters):
    ...
    data_normalized = [a for a in zip(x_pos_normalized, velocity_normalized)]
    ...
    returned_data = [a for a in zip(data[:, 0], data[:, 1])]
...
def get_cluster_fn(env_name='MountainCar-v0', n_clusters=10, extra_bit=True, load_from_dir=False):
    if load_from_dir:
